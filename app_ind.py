    ...
    while True:
        with storage_lock:
            ...
            for uid in expired_uids:
                if jwt_token:  
                    requests.get(f"http://168.231.113.96:1882/remove_friend?token={jwt_token}&id={uid}&key={key2}")
                del uids[uid]
...
def add_uid():
    ...
    if permanent:
        ...
        if jwt_token: 
            requests.get(f"http://168.231.113.96:1882/adding_friend?token={jwt_token}&id={uid}&key={key2}")
    else:
